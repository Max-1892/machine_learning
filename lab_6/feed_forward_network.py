...
class FeedForwardNetwork:
    def __init__(self, learning_rate, num_of_attrs, 
        ...
        '''This method initializes the instances variables.
           ...
           Note:
               +Nodes of the network are represented here as dictionaries
               +Layers are represented as arrays of nodes and so arrays of dictionaries
               +The whole network is therefore an array of layers 
        ...
        for layer_idx in xrange(num_of_hidden_layers):
            layer = list()
            for node_idx in xrange(num_of_hidden_nodes_per_layer[layer_idx]):
                if layer_idx == 0:
        ...
        if num_of_hidden_layers > 0:
           for layer_idx in xrange(self.num_of_outputs):
    ...
    def determine_node_outputs(self, data_instance):
        '''This method determines the each node's output 
           ...
           function.
        '''
        # Inputs to each node change after the first hidden layer
        # so store the last layer's output here. It is initialized
        # to the data instance for which we are trying to determine the
        # networks output.
        current_layer_input = data_instance
        for layer in self.network:
            next_layers_input = list()
            for node in layer:
                # Calculate the node's output with the dot product
                # of the weights and then add on the bias.
                # The output of the node is stored at the node
                # to help with when backprop is applied.
    ...
    def backprop_error(self, targeted_network_value):
        '''This method calculates the error each node is
           ...
           1. Calculate the errors of the output nodes
               error_output_node = output * (1 - output) * (target - output)

           2. Calculate the errors for each hidden layer node
               hidden_layer_node_error = 
                   hidden_layer_output * (1 - hidden_layer_output) * 
                   sum_over_downstream_nodes[
                       (downstream_node_error * weight_of_connection_to_downstream_node)]
        '''
        # Starting at the output layer, calculate the error each node
        # is responsible for
        output_layer_index = len(self.network) - 1
        for layer_idx in reversed(xrange(len(self.network))):
    ...
    def train(self, training_set):
        '''This method trains the weights of the network using
           ...
           +Update the weights from the last hidden layer 
              to the output layer. A single weight from a hidden layer
              to a node in the output layer is updated according to the following:
              new_weight = 
                  old_weight + learning_rate * error_output_node * hidden_layer_node_output

           +Update the weights from the upstream nodes to each hidden layer node
               new_weight =
                   old_weight + learning_rate * hidden_layer_node_error * upstream node outputs
        '''
        trials = 0
        while trials < 250:
            for instance in training_set:
                ...
                self.backprop_error( \
                    [1 if (instance[-1] - 1) == idx else 0 for idx in xrange(self.num_of_outputs)])
                # Now go through the network and update weights according to: 
                #     if connection terminates at an output node:
                #         new_weight = 
                #             old_weight + learning_rate * output_node_error * hidden_node_output
                #     if connection terminates at another hidden node:
                #        new_weight =
                #           old_weight + learning_rate * hidden_node_error * upstream_node_outputs
                for layer_idx, layer in enumerate(self.network):
                    for node in layer:
                        if layer_idx == 0:
                            # first hidden layer
                            for weight_idx in xrange(len(node['weights']) - 1):
    ...
    def predict(self, test_set):
        ...
        for instance in test_set:
            ...
            outputs = [self.network[-1][i]['output'] for i in xrange(self.num_of_outputs)]
