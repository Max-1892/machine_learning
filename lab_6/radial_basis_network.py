...
class RadialBasisNetwork:
    def __init__(self, learning_rate, spread, num_of_outputs):
        ...
        '''Output layer - single node for each class'''
    ...
    def train(self, training_data):
       ...
       for _ in xrange(self.num_of_outputs):
           self.weights.append(
               np.array([np.random.randint(-1, 1) for weight in xrange(len(self.hidden_nodes) + 1)]))
       ...
       while trials < 250:
           ...
           new_weights = [np.array([0.0 for i in xrange(len(self.hidden_nodes) + 1)]) for _ in xrange(self.num_of_outputs)]
           for instance in training_data:
               ...
               for output_idx in xrange(self.num_of_outputs):
                   gradient = []
           ...
           new_weights = [new_weights[i] / training_data.shape[0] for i in xrange(self.num_of_outputs)]
    ...
    def predict(self, test_set):
        ...
        for instance in test_set:
            ...
            gaussian_outputs.append(1)
            outputs = list()
            for weight_idx in xrange(self.num_of_outputs):
