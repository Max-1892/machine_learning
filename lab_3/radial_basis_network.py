...
class RadialBasisNetwork:
    def __init__(self, learning_rate, learner_type):
        ...
        '''Spread = 2 * average distance to centroid'''
    ...
    def train(self, training_data, do_clustering = False, num_of_means = -1):
       '''
           1. Find hidden nodes
               a. apply k-means
               b. define one hidden node per cluster
               c. use centroid as x_j
                   i. exp[(-1/(2 * spread **2)) * (instance - centroid)**2]
               d. spread = 2 * d_ave
           2. Learn weights in output function
       ...
       if do_clustering:
           ...
           # Determine the spread for each hidden node, spread = 2 * ave_dist_to_other_centroids
           ...
           # Determine the spread for each hidden node, 
           # spread = 2 * dist_ave_to_other_training_instances
       ...
       self.weights = \
           np.array([np.random.randint(-100, 100) for weight in xrange(len(self.hidden_nodes))])
