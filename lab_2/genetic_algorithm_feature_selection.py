...
def init_population(size_of_population, num_of_features):
    ...
    while len(feature_sets_to_accuracy) < size_of_population:
        ...
        candidate_feature_set = \
            [1 if random.random() > 0.5 else 0 for feature in xrange(num_of_features)]
...
def select_parents(feature_sets_to_accuracy):
    ...
    parents = np.random.choice(xrange(len(candidate_feature_sets)), size=2, replace=True, p=fitness_proportions)
...
def mutate(offspring):
    ...
    mutated_offspring = [abs(offspring[idx] - 1) if random.random() <= 0.01 else offspring[idx] for idx in xrange(len(offspring))]
...
def perform_GA_feature_selection(model, model_type, num_of_classes, data_set):
    ...
    for candidate_idx in xrange(len(feature_sets_to_accuracy)):
