...
class KMeans:
    ...
    def cluster(self, data):
        ...
        '''Initialize means with by randomly selecting an attribute value
        between the min and max value seen in each attribute.'''
        self.means = []
        ...
        '''As long as the means are changing by some amount, 
        continue running.'''
        means_updating = True
        loops = 0
        while means_updating and loops < 1000:
            ...
            '''NOTE: Euclidean distance is used.'''
            ...
            '''Recalculate means by cycling through the instances belonging to each
        ...
        for mean_1 in self.means:
            for mean_2 in self.means:
                if not mean_1 == mean_2:
                    for mean_1_val, mean_2_val in zip(mean_1, mean_2):
                        total += (mean_1_val - mean_2_val) ** 2
        return total 
    ...
    def calculate_inner_class_scatter(self):
        ...
        for mean_idx in xrange(len(self.means)):
            for instance in self.clusters[mean_idx]:
                for inst_val, mean_val in zip(instance, self.means[mean_idx]):
