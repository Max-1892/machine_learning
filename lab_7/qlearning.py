...
def qlearning(grid_map, map_height, map_width, start_locs, learning_rate, discount_factor, epsilon, crash_type, track_name):
   ...
   for state_to_visit in states_to_visit:
       ...
       while grid_map[state[0][0],state[0][1]] != 'F':
           ...
           if np.random.choice(['greedy','random'], [1.0-epsilon, epsilon]) == 'greedy':
               ...
               action = reachable_state_values[0][0]
           else:
           ...
           successor_action = \
               calculate_reachable_state_values(
                   grid_map, q_value_map, successor_state, possible_actions, crash_type, start_locs)[0][0]
           ...
           # Q_t+1(s,a) = Q(s,a) + learning_rate * (reward + discount_factor * max_a' Q(s',a')-Q(s,a))
